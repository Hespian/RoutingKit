...
public:
    NodeContractionGraph(unsigned node_count, const std::vector<unsigned> &tail, const std::vector<unsigned> &head)
        : g(node_count, tail, head)
        , is_virtual(node_count) {
    ...
    void forall_neighbors_then_contract_node(int v, const F &callback) {
        ...
        g.forall_nodes_in_last_computed_neighborhood([&](unsigned u) {
            if(is_virtual[u])
...
unsigned compute_chordal_supergraph(unsigned node_count,
    ...
    for(unsigned x = 0; x < node_count - 1; ++x) {
        ...
        g.forall_neighbors_then_contract_node(x, [&](unsigned y) {
            on_new_arc(x, y);
            ++upward_degree;
        });
...
CustomizableContractionHierarchy::CustomizableContractionHierarchy(std::vector<unsigned> arg_order,
    ...
    {
        input_arc_id =
            compute_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(node_count, input_tail, input_head);
    ...
    }
    {
        ...
        {
            auto p = compute_inverse_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(node_count,
                                                                                                        symmetric_tail,
                                                                                                        symmetric_head);
    ...
    {
        down_to_up =
            compute_sort_permutation_first_by_tail_then_by_head_and_apply_sort_to_tail(node_count, down_tail, down_head);
...
CustomizableContractionHierarchyMetric::CustomizableContractionHierarchyMetric(const CustomizableContractionHierarchy &cch,
                                                                               const std::vector<unsigned> &input_weight)
    : forward(cch.cch_arc_count())
    , backward(cch.cch_arc_count())
    , cch(&cch)
...
CustomizableContractionHierarchyMetric &
CustomizableContractionHierarchyMetric::reset(const CustomizableContractionHierarchy &cch_, const unsigned *input_weight_) {
...
CustomizableContractionHierarchyPartialCustomization::reset(const CustomizableContractionHierarchy &cch_) {
    ...
    while(!q.empty()) {
        ...
        if(old_forward != new_forward || old_backward != new_backward) {
            forall_intermediate_triangles_of_arc(
                *cch,
                xy,
                [&](unsigned bottom_arc, unsigned mid_arc, unsigned top_arc, unsigned bottom_node, unsigned mid_node, unsigned top_node) {
                    assert(mid_arc == xy);
...
void forall_ancestors(const std::vector<unsigned> &parent, unsigned x, unsigned stop_at, const F &f) {
    ...
    while(x != stop_at) {
        if(CustomizableContractionHierarchyQuery::count)
            ++CustomizableContractionHierarchyQuery::numVerticesSettled;
...
void relax_outgoing_arcs(const std::vector<unsigned> &first_out,
    ...
    for(unsigned xy = first_out[x]; xy < first_out[x + 1]; ++xy) {
        unsigned y = head[xy];
        if(CustomizableContractionHierarchyQuery::count)
            ++CustomizableContractionHierarchyQuery::numEdgesRelaxed;
...
unsigned CustomizableContractionHierarchyQuery::numVerticesSettled = 0;
unsigned CustomizableContractionHierarchyQuery::numEdgesRelaxed = 0;
bool CustomizableContractionHierarchyQuery::count = false;
...
unsigned unpack_shortest_path(const CustomizableContractionHierarchy &cch,
    ...
    }
    {
        ...
        while(y != invalid_id) {
            unpack_arc(cch, metric, false, y, x, find_arc_given_sorted_head(cch.up_first_out, cch.up_head, y, x), on_new_segment);
            x = y;
...
CustomizableContractionHierarchyQuery::pin_targets(const std::vector<unsigned> &target_list) {
    assert(state == query_state_initialized);
...
CustomizableContractionHierarchyQuery &
CustomizableContractionHierarchyQuery::pin_sources(const std::vector<unsigned> &source_list) {
    assert(state == query_state_initialized);
...
CustomizableContractionHierarchyQuery &CustomizableContractionHierarchyQuery::reset() {
    if(state == query_state_target_pinned || state == query_state_target_run) {
        reset_target_distances(cch->elimination_tree_parent,
                               target_node,
                               target_elimination_tree_end,
                               forward_tentative_distance);
